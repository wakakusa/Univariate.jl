...
@testset "univariate.jl" begin
    ...
    SepalLength=map(Int,10*SepalLength)
    SepalWidth=map(Int,10*SepalWidth)
    PetalLength=map(Int,10*PetalLength)
    PetalWidth=map(Int,10*PetalWidth)

    INPUT=DataFrame(SepalLength=SepalLength,SepalWidth=SepalWidth,PetalLength=PetalLength,PetalWidth=PetalWidth,Species=Species)
